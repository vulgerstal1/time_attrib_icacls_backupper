#vulgerstal's taicacls_b (timestamp, attribute, icacls backupper) 17g24 8:28P
...
#================================================================== SETTINGS ===================================================================================================================
$Log_default = 'C:\Dev_VTAIB\vtaib_lastSession.vlog'
$Mode = 0;
$Target_default = 'C:\Dev_Unity_Projects\game_press'
$ContentToPick_default = '*.*'
$BackupTimeAndAttributes_default = 'C:\Dev_VTAIB\folder.vtab'
$BackupSecurity_default = 'C:\Dev_VTAIB\folder.vicb'
...
$Mode = Read-Host "LEAVE EMPTY AND PRESS ENTER TO ITEM+CONTENT, 1 for ITEM ONLY, 2 for CONTENT ONLY"
if($Mode -eq '') { $Mode = 0 }
$ContentToPick = Read-Host "Set ContentToPick, or LEAVE EMPTY and PRESS ENTER for $ContentToPick_default"
...
if($NoSubDirs_bool -eq $false) { Set-Location $Target } else { Set-Location (Split-Path $Target -Parent) }
$userRequest = Read-Host "PRESS ENTER ANYTHING and PRESS ENTER to BACKUP"
if($userRequest -ne '')
{	
	$userRequest = ''		
	if($Mode -eq 0) #everything
	{
		...
		Get-ChildItem $Target -recurse -Force -Filter $ContentToPick | Select-Object -Property FullName, CreationTime, LastWriteTime, LastAccessTime, Attributes | Export-Csv -NoTypeInformation -append $BackupTimeAndAttributes
	}
	if($Mode -eq 1) #folder
	{
		Get-Item $Target -force -Filter $ContentToPick | Select-Object -Property FullName, CreationTime, LastWriteTime, LastAccessTime, Attributes | Export-Csv -NoTypeInformation $BackupTimeAndAttributes
	}
	if($Mode -eq 2) #content
	{
		Get-ChildItem $Target -recurse -Force -Filter $ContentToPick | Select-Object -Property FullName, CreationTime, LastWriteTime, LastAccessTime, Attributes | Export-Csv -NoTypeInformation $BackupTimeAndAttributes
	}	
	...
	if($Mode -eq 1) #folder
	{
		icacls (Get-Item $Target -Force) /save $BackupSecurity /c
	}
	if($Mode -eq 2) #content
	{
		icacls (Get-ChildItem $Target -recurse -Force -Filter $ContentToPick) /save $BackupSecurity /t /c
	}
...
{	
	$userRequest = ''	
	$Target_now = $Target
	$Target = Read-Host "Set Target, or LEAVE EMPTY and PRESS ENTER for $Target or PRESS SPACE and ENTER for $Target_default"
